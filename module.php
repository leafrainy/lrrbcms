...
function get_list($sort){
	...
	<div class="boxt">
	<span><a href="<?php echo Url::sort($sort);?>">更多...</a></span>
	<strong><a href="<?php echo Url::sort($sort);?>"><?php echo $sortname['sortname'];?></a></strong>
	</div>
	<ul>
	<?php
	$sql2 = "SELECT gid,title,date FROM ".DB_PREFIX."blog WHERE sortid=".$sort." AND hide='n' ORDER BY `date` DESC LIMIT 10";
	$list = $db->query($sql2);
	while($row = $db->fetch_array($list)){
	?>
    <li>
	<a href="<?php echo Url::log($row['gid']);?>" title="<?php echo $row['title'];?>"><?php echo $row['title'];?></a><span><?php echo gmdate('Y-m-d', $row['date']);?></span>
	</li>
	<?php }?>
	</ul>
	</div>
